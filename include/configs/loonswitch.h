...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"bootcmd=" \
		...
		"run bootfromsd; " \
		"elif test \"${bootdevice}\" = \"emmc\"; then " \
		"echo && echo Google Loonswitch -- Booting from eMMC... && " \
		"run bootfromemmc; " \
		"fi\0" \
	"prepemmc=ums 0 mmc 1\0" \
	"prepsd=ums 0  mmc 0\0" \
	"updateflashfromemmc=if fatload mmc 1 ${uimage_addr} u-boot.imx; then " \
		"sf probe && sf erase 0 0xc0000 && sf write ${uimage_addr} 0x400 $filesize && " \
		"echo flash is now updated ; fi\0" \
	"updateflashfromsd=if fatload mmc 0 ${uimage_addr} u-boot.imx; then " \
		"sf probe && sf erase 0 0xc0000 && sf write ${uimage_addr} 0x400 $filesize && " \
		"echo flash is now updated ; fi\0" \
...
#define CONFIG_SYS_MEMTEST_END	       0x70000000
#define CONFIG_SYS_MEMTEST_SCRATCH     0x70800000
...
/* Netchip IDs */
#define CONFIG_G_DNL_VENDOR_NUM 0xffff
